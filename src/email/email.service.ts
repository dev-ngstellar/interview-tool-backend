...
export class EmailService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      if (verification.success) {
        ...
        // If SMTP authentication succeeds, perform initial SMTP acceptance test
        try {
          const testRecipient = "bharathsurya539@gmail.com";
          const senderAddress =
            this.smtpConfig.fromEmail || this.smtpConfig.user;
          const startupSendResult = await this.transporter!.sendMail({
            from: `"${this.smtpConfig.fromName}" <${senderAddress}>`,
            to: testRecipient,
            envelope: {
              from: senderAddress,
              to: testRecipient,
          ...
          this.logSafeSmtpResponse(
            startupSendResult,
            testRecipient,
          );
        } catch (sendErr: any) {
          this.logger.error(
            `Failed to send startup test email: ${sendErr?.message}`,
          );
        }
      } else {
  ...
  /**
   * Development-only test email dispatcher (Section 8).
   * Uses the EXACT SAME EmailService configuration as OTP sending.
   * Sends:
   *   From: <GMAIL> ("Marketing Executive Recruitment <GMAIL>")
   *   To: a configurable test recipient (default: bharathsurya539@gmail.com)
   *   Subject: Marketing Executive SMTP Test
   *   Body: This is a test email from the Marketing Executive Recruitment Application.
   * Returns: messageId, accepted, rejected, response
   */
  async sendTestEmail(to: string = "bharathsurya539@gmail.com"): Promise<{
    success: boolean;
    message: string;
    recipient: string;
    messageId?: string;
    ...
    try {
      ...
      // Verify connection first using the exact same configuration
      await this.transporter.verify();

      this.logger.log(`SMTP test recipient: ${to}`);

      const subject = "Marketing Executive SMTP Test";
      const text =
        "This is a test email from the Marketing Executive Recruitment Application.";
      const html =
        "<p>This is a test email from the Marketing Executive Recruitment Application.</p>";
      ...
      const rejected = Array.isArray(sendResult.rejected)
        ? sendResult.rejected.map((r: any) => String(r))
        : [];
      const isRejected = rejected.includes(to);
      ...
      if (isRejected) {
        await this.prisma.emailLog.create({
          data: {
            ...
            template: "ADMIN_TEST",
            status: EmailStatus.FAILED,
            errorMessage: `SMTP server rejected recipient: ${to}`,
          },
        });

        return {
          success: false,
          message: `SMTP rejected recipient: ${to}`,
          recipient: to,
      ...
      const code = error?.code || (isAuthError ? "EAUTH" : "UNKNOWN");
      if (isAuthError) {
        this.logAuthFailure(code);
      }

      const safeCategory = isAuthError
        ? "SMTP server rejected the supplied credentials."
        : this.categorizeSmtpError(error);
      ...
      return {
        success: false,
        message: `SMTP test failed: ${safeCategory}`,
        recipient: to,
        messageId: undefined,
        accepted: [],
        rejected: [to],
        response: safeCategory,
        code,
        error: safeCategory,
      };
    }
